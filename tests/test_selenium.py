...
def test_click_the_button(selenium_driver: Chrome, server: Server) -> None:
    selenium_driver.get(server.PREFIX + "/input/button.html")
    sync_locator = selenium_driver.find_element(By.XPATH, "//button")
    x, y = get_locator_pos(sync_locator)
    selenium_driver.sync_input.click("left", x, y)  # type: ignore[attr-defined]
...
def test_double_click_the_button(selenium_driver: Chrome, server: Server) -> None:
    selenium_driver.get(server.PREFIX + "/input/button.html")
    selenium_driver.execute_script(
        """window.double = false;
...
def test_locators_hover(selenium_driver: Chrome, server: Server) -> None:
    ...
    selenium_driver.get(server.PREFIX + "/input/scrollable.html")
    selenium_driver.sync_input.move(500, 100)  # type: ignore[attr-defined]

    sync_locator = selenium_driver.find_element(By.ID, "button-12")
    x, y = get_locator_pos(sync_locator)
...
def test_fill_input(selenium_driver: Chrome, server: Server) -> None:
    selenium_driver.get(server.PREFIX + "/input/textarea.html")
    sync_locator = selenium_driver.find_element(By.XPATH, "//input")
    assert sync_locator

    x, y = get_locator_pos(sync_locator)
    selenium_driver.sync_input.click("left", x, y)  # type: ignore[attr-defined]
    selenium_driver.sync_input.type("some value", fill=True)  # type: ignore[attr-defined]
    assert selenium_driver.execute_script("return result") == "some value"


def test_keyboard_type_into_a_textarea(selenium_driver: Chrome) -> None:
    selenium_driver.execute_script(
        """
    ...
    sync_locator = selenium_driver.find_element(By.XPATH, "//textarea")
    assert sync_locator
...
def test_quit_exception(selenium_driver: Chrome) -> None:
    ...
    with pytest.raises(WindowClosedException):
        selenium_driver.sync_input.up("left", 110, 110)  # type: ignore[attr-defined]
    with pytest.raises(WindowClosedException):
        selenium_driver.sync_input.move(50, 50, emulate_behaviour=False)  # type: ignore[attr-defined]
    with pytest.raises(WindowClosedException):
        selenium_driver.sync_input.scroll("up", 10)  # type: ignore[attr-defined]
    with pytest.raises(WindowClosedException):
        selenium_driver.sync_input.type("test")  # type: ignore[attr-defined]
