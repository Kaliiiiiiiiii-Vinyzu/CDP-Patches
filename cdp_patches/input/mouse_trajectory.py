...
class HumanizeMouseTrajectory:
    ...
    def generate_curve(self) -> List[Tuple[float, float]]:
        ...
        points_distance = math.sqrt(
            (self.from_point[0] - self.to_point[0]) ** 2
            + (self.from_point[1] - self.to_point[1]) ** 2
        )
    ...
    def generate_internal_knots(
        ...
        std_deviation = min(
            midpoint_x - l_boundary,
            r_boundary - midpoint_x,
            midpoint_y - d_boundary,
            u_boundary - midpoint_y,
        )

        # Generate knotsX and knotsY using normal distribution
        knotsX = np.random.normal(midpoint_x, std_deviation, size=knots_count).astype(
            int
        )
        knotsY = np.random.normal(midpoint_y, std_deviation, size=knots_count).astype(
            int
        )
        ...
        def distance(knot):
            return math.sqrt(
                (self.from_point[0] - knot[0]) ** 2
                + (self.from_point[0] - knot[1]) ** 2
            )
    ...
    ) -> Union[List[Tuple[float, float]], NoReturn]:
    ...
    def tween_points(
        self, points: List[Tuple[float, float]], target_points: int
    ) -> Union[List[Tuple[float, float]], NoReturn]:
    ...
    def check_if_list_of_points(
        self, list_of_points: Union[List[Tuple[int, int]], List[Tuple[float, float]]]
    ) -> bool:
...
class BezierCalculator:
    ...
    def bernstein_polynomial_point(x: int, i: int, n: int) -> float:
        ...
        return float(
            BezierCalculator.binomial(n, i) * (x**i) * ((1 - x) ** (n - i))
        )  # Quirky for mypy :D
